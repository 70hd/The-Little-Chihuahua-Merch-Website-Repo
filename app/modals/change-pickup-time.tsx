...
const ChangePickupTime = ({
  ...
  const handleSubmit = () => {
    ...
    if (!formData.location) {
      setError((prev) => ({
        ...prev,
        location: "Must select a pickup location",
      }));
      hasError = true;
    } else {
    ...
    const matchingHours =
      options[0]?.options?.find((opt) => opt.location === formData.location)
        ?.hours || "";
    const formattedDate = formatDate(date);

    setPickupDetails(
      `${formData.location} ${matchingHours}`,
      formattedDate,
      false
    );
    setLoading(false);
  ...
  return (
    <div
      className={`absolute max-w-[392px] w-full top-50 h-full z-50 flex items-center justify-center ${
        checkout ? "top-[185px] sm:left-[54px]" : "top-12 left-[0px]"
      }`}
    >
      <div className="w-full sm:min-w-[392px] h-fit flex flex-col gap-6 p-6 bg-white border-black/75 border-[.5px] sm:px-8 px-4">
        <div className="w-full flex justify-between">
          <h3 id="modal-heading" className="text-xl font-semibold">
            Customize Pickup Time
          </h3>
          <button
              ...
              {!selectOption.onClick ? (
                ...
                <div className="flex flex-col gap-3 w-full ">
                  {selectOption.name === "date" && error.date && (
                    <p className="text-[#CD3626]" id="date-error">
                      {error.date}
                    </p>
                  )}
                  <CustomDatePicker
